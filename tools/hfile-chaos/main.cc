...
void print_usage()
{
    std::cerr << "Usage: hfile-chaos --mode <kill-during-write|disk-full-sim> "
...
std::optional<Options> parse_args(int argc, char** argv)
{
    ...
    if (opts.mode.empty() || opts.output_dir.empty())
    {
        return std::nullopt;
    }
    return opts;
...
hfile::Status write_workload(
    const fs::path& hfile_path, size_t rows, size_t value_size, const hfile::WriterOptions& options, bool slow_mode)
{
    ...
    for (size_t i = 0; i < rows; ++i)
    {
        ...
        auto append_status = writer->append(row, family, qualifier, static_cast<int64_t>(rows - i), value);
        if (!append_status.ok())
        {
...
int run_disk_full_sim(const Options& opts)
{
    ...
    {
        std::cerr << "Expected DISK_SPACE_EXHAUSTED, got: " << (status.ok() ? "OK" : status.message()) << "\n";
        return 3;
    }

    if (opts.verify_no_corrupt_files && !verify_output_is_clean(opts.output_dir, hfile_path))
    {
        return 4;
    }
