...
{
public:
    explicit ManualSplitPartitioner(std::vector<std::vector<uint8_t>> splits) : splits_(std::move(splits))
    {
    ...
    int region_for(std::span<const uint8_t> row_key) const noexcept override
    {
        ...
        // call — the original code allocated O(key_size) bytes per call,
        // which
        // was the dominant allocation on the hot Bulk Load write path.
        auto cmp = [](const std::vector<uint8_t>& split, std::span<const uint8_t> key) noexcept -> bool
        {
        ...
        auto it = std::upper_bound(splits_.begin(),
                                   splits_.end(),
                                   row_key,
                                   [](std::span<const uint8_t> key, const std::vector<uint8_t>& split) noexcept -> bool
                                   {
...
std::unique_ptr<RegionPartitioner> RegionPartitioner::from_splits(std::vector<std::vector<uint8_t>> split_points)
{
