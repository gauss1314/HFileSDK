...
uint32_t quota_to_worker_limit(uint64_t quota, uint64_t period) noexcept {
    ...
    return static_cast<uint32_t>(std::clamp<uint64_t>(
        workers, 1, std::numeric_limits<uint32_t>::max()));
}
...
uint32_t read_cgroup_v1_cpu_limit_at(const std::string &directory) {
    ...
    int64_t quota = -1;
    int64_t period = 0;
    if (!(quota_file >> quota) || !(period_file >> period) || quota <= 0 ||
        period <= 0) {
        return 0;
    }
    return quota_to_worker_limit(static_cast<uint64_t>(quota),
...
uint32_t read_hierarchical_limit(const std::string &mount_root,
                                 std::string_view cgroup_path,
...
bool controller_list_has_cpu(std::string_view controllers) noexcept {
    ...
    while (begin <= controllers.size()) {
        ...
        const size_t end = comma == std::string_view::npos
            ? controllers.size()
            : comma;
...
uint32_t read_cgroup_cpu_limit() {
    ...
    while (std::getline(membership, line)) {
        ...
        const std::string_view controllers(
            line.data() + first + 1, second - first - 1);
    ...
    apply_limit(read_hierarchical_limit(
                    "/sys/fs/cgroup", v2_path,
                    read_cgroup_v2_cpu_limit_at),
                &limit);
    ...
    for (const auto mount : kV1CpuMounts) {
        apply_limit(read_hierarchical_limit(
                        std::string(mount), v1_path,
                        read_cgroup_v1_cpu_limit_at),
                    &limit);
    }
    return limit;
}
...
uint32_t available_cpu_limit() noexcept {
    ...
    try {
        ...
        if (::sysctlbyname("hw.physicalcpu", &physical, &physical_size, nullptr,
                           0) == 0 &&
            physical > 0) {
...
CompressionExecutor::CompressionExecutor()
    : queue_(kQueueCapacity), worker_limit_(available_cpu_limit()) {}
...
bool CompressionExecutor::submit(Task task) {
    ...
    space_cv_.wait(
        lock, [this]() { return stopping_ || queue_size_ < queue_.size(); });
    if (stopping_)
        return false;
...
void CompressionExecutor::worker_loop() noexcept {
    ...
    for (;;) {
        Task task;
        {
            std::unique_lock<std::mutex> lock(mu_);
            work_cv_.wait(lock,
                          [this]() { return stopping_ || queue_size_ > 0; });
        ...
        try {
            if (task.compression == Compression::GZip) {
                auto &cached = gzip_compressors[compressor_cache_index(
                    task.compression_level)];
                if (!cached) {
                    cached = Compressor::create(task.compression,
                                                task.compression_level);
                }
