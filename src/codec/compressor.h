...
public:
    ...
    virtual size_t compress(std::span<const uint8_t> input, uint8_t* output, size_t output_capacity) const noexcept = 0;
    ...
    /// `compress()`.
    virtual size_t compress_with_crc32(std::span<const uint8_t> input,
                                       uint8_t* output,
    ...
    virtual Status decompress(std::span<const uint8_t> compressed, uint8_t* output, size_t output_size) const noexcept
        = 0;

    Compression type() const noexcept
    {
        return type_;
    }
