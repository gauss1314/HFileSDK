...
public:
    ...
    size_t compress(std::span<const uint8_t> input, uint8_t* output, size_t capacity) const noexcept override
    {
    ...
    Status decompress(std::span<const uint8_t> compressed, uint8_t* output, size_t output_size) const noexcept override
    {
        if (output_size < compressed.size())
        {
            return Status::InvalidArg("buffer too small");
        }
...
{
public:
    /// @param level 1 (fastest) to 9 (best ratio).  0 = Z_DEFAULT_COMPRESSION
    /// (6).
    explicit GZipCompressor(int level = Z_DEFAULT_COMPRESSION)
        : Compressor(Compression::GZip), level_(level <= 0 ? Z_DEFAULT_COMPRESSION : level)
    {
    }
...
private:
                         ...
                         uint32_t precomputed_crc32) const noexcept
    {
        ...
        if (capacity < sizeof(kGzipHeader) + 8)
        {
            return 0;
        }
...
public:
    Status decompress(std::span<const uint8_t> compressed, uint8_t* output, size_t output_size) const noexcept override
    {
        ...
        if (ret != Z_OK)
        {
            return Status::Corruption("inflateInit2 failed");
        }
        ...
        for (;;)
        {
            ...
            inflateEnd(&strm);
            return Status::Corruption("GZip decompress failed (inflate returned " + std::to_string(ret) + ")");
        }
        ...
        {
            return Status::Corruption("GZip decompressed size mismatch (expected " + std::to_string(output_size)
                                      + ", got " + std::to_string(total_out) + ")");
        }
        return Status::OK();
    }
...
private:
    ...
    bool ensure_initialized() const noexcept
    {
        ...
        const int ret = deflateInit2(&strm_,
                                     level_,
                                     Z_DEFLATED,
                                     -MAX_WBITS,
                                     /*memLevel=*/8,
