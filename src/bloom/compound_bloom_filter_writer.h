...
{
    return value < 0x80u ? static_cast<uint32_t>(value) : (0xFFFFFF00u | static_cast<uint32_t>(value));
}

template <typename ByteAt> inline int32_t murmur_hash_bytes(size_t len, ByteAt byte_at, int32_t seed) noexcept
{
    ...
    for (size_t i = 0; i < len_4; ++i)
    {
        ...
        uint32_t k = static_cast<uint32_t>(byte_at(i_4 + 0)) | (static_cast<uint32_t>(byte_at(i_4 + 1)) << 8)
...
/// HBase-compatible MurmurHash (same algorithm as
...
inline int32_t
murmur_hash_row_col(std::span<const uint8_t> row, std::span<const uint8_t> qualifier, int32_t seed = 0) noexcept
{
    const detail::RowColBloomKeyView key{row, qualifier};
    return detail::murmur_hash_bytes(
        key.size(),
        [&key](size_t offset) noexcept
        {
            return key(offset);
        },
        seed);
...
public:
    ...
    static constexpr size_t kMaxChunkSize = size_t{128} * 1024; // 128 KB per chunk
    ...
    CompoundBloomFilterWriter(BloomType type = BloomType::Row,
        ...
        : type_{type}, error_rate_{error_rate}, comparator_class_name_{std::move(comparator_class_name)}
    {
        if (type == BloomType::None)
        {
            return;
        }
    ...
    {
        if (type_ == BloomType::None)
        {
            return;
        }
        if (cur_keys_ >= chunk_keys_)
        {
            finish_chunk();
        }
        if (cur_keys_ == 0)
    ...
    void add_row_col(std::span<const uint8_t> row, std::span<const uint8_t> qualifier)
    {
    ...
    /// and must be called explicitly by the writer when a data block is
    /// finalized).
    void finish_chunk()
    {
        if (type_ == BloomType::None || cur_keys_ == 0)
        {
            return;
        }
        compact_chunk();
    ...
    {
        if (type_ == BloomType::None || chunks_.empty())
        {
            return false;
        }
        ...
        for (size_t i = 0; i < flush_count; ++i)
        {
            ...
            chunk_on_disk_sizes_.push_back(static_cast<uint32_t>(blocks_out.size() - before_size));
                            ...
                            codec::Compressor* compressor = nullptr)
    {
        ...
        return flush_ready_data_blocks(blocks_out, current_file_offset, prev_block_offset, result, compressor);
    }

    /// Serialise the BLMFMET2 bloom meta block into `meta_out`.
    /// Must be called after finish_data_blocks().
    /// `meta_block_file_offset` is the absolute file offset where this block
    /// starts (used to build the meta root index entry pointing to BLMFMET2).
    ...
    {
        return type_ != BloomType::None;
    }
    bool has_data() const noexcept
    {
        return !chunks_.empty() || !chunk_offsets_.empty() || cur_keys_ > 0;
    }
    bool has_ready_chunks() const noexcept
    {
        return !chunks_.empty();
    }
    size_t ready_chunk_count() const noexcept
    {
        return chunks_.size();
    }
    uint32_t total_keys() const noexcept
    {
        return total_keys_;
    }
    BloomType bloom_type() const noexcept
    {
        return type_;
    }

    /// Legacy combined API (chunk blocks + meta in one buffer).
    /// Retained for unit tests that only care about serialised content, not
    /// layout.
    BloomWriteResult finish(std::vector<uint8_t>& meta_blocks_out, int64_t current_offset)
    {
        BloomWriteResult result;
        ...
        if (!finish_data_blocks(meta_blocks_out, current_offset, 0, &result))
        {
            return result;
        }
        ...
        result.bloom_meta_offset = current_offset + static_cast<int64_t>(meta_blocks_out.size());
...
private:
    ...
    void set_bits(int32_t hash1, int32_t hash2) noexcept
    {
        ...
        for (int i = 0; i < hash_count_; ++i)
        {
            ...
            if (power_of_two) [[likely]]
            {
                ...
                const uint32_t magnitude = signed_hash < 0 ? 0u - composite_hash : composite_hash;
    ...
    void compact_chunk()
    {
        if (cur_chunk_.empty() || cur_keys_ == 0)
        {
            return;
        }
        size_t pieces = 1;
    ...
    {
        return static_cast<long long>(static_cast<double>(bit_size) * (kLog2Squared / -std::log(error_rate)));
    }

    static long long compute_max_keys(long long bit_size, double error_rate, int hash_count)
    {
        return static_cast<long long>(-static_cast<double>(bit_size) / hash_count
    ...
    static int compute_optimal_function_count(int max_keys, long long bit_size)
    {
        ...
        return std::max(1, static_cast<int>(std::ceil(std::log(2.0) * static_cast<double>(ratio))));
    }
                                 ...
                                 codec::Compressor* compressor = nullptr)
    {
        ...
        if (compressor && compressor->type() != Compression::None)
        {
            ...
            size_t comp_len
                = compressor->compress({chunk_data.data(), chunk_data.size()}, compressed.data(), compressed.size());
            if (comp_len > 0)
            {
        ...
        std::memcpy(p, kBloomChunkMagic.data(), 8);
        p += 8;
        write_be32(p, static_cast<uint32_t>(payload.size() + checksum_buf.size()));
        p += 4;
        write_be32(p, static_cast<uint32_t>(chunk_data.size()));
        p += 4; // uncompressed original size
        write_be64(p, static_cast<uint64_t>(prev_block_offset));
        p += 8;
        *p++ = kChecksumTypeCRC32C;
        write_be32(p, kBytesPerChecksum);
        p += 4;
        write_be32(p, static_cast<uint32_t>(on_disk_data_with_header));
        p += 4;
        ...
        std::memcpy(out.data() + off + kBlockHeaderSize + payload.size(), checksum_buf.data(), checksum_buf.size());
    }
                                ...
                                codec::Compressor* compressor = nullptr)
    {
        ...
        // version(4B BE) + totalByteSize(8B BE) + hashCount(4B BE) +
        // hashType(4B BE)
        ...
        //   [chunkOffset(8B BE) + chunkByteSize(4B BE) + firstKey(bytes with
        //   writable-vint length prefix)] * N

        // -- Step 1: serialize bloom meta data into temp buffer
        // ----
        uint8_t comparator_len_buf[10];
        const int comparator_len_size
            = encode_writable_vint(comparator_len_buf, static_cast<int64_t>(comparator_class_name_.size()));
        size_t index_payload_size = 0;
        for (const auto& first_key : chunk_first_keys_)
        {
            uint8_t key_len_buf[10];
            const int key_len_size = encode_writable_vint(key_len_buf, static_cast<int64_t>(first_key.size()));
            index_payload_size += 8 + 4 + static_cast<size_t>(key_len_size) + first_key.size();
        }
        size_t data_size = 4 + 8 + 4 + 4 + 8 + 8 + 4 + static_cast<size_t>(comparator_len_size)
                           + comparator_class_name_.size() + index_payload_size;

        std::vector<uint8_t> raw_data(data_size);
        uint8_t* dp = raw_data.data();

        write_be32(dp, kBloomFormatVersion);
        dp += 4;
        uint64_t total_bytes = 0;
        for (uint32_t size : chunk_byte_sizes_)
        {
            total_bytes += size;
        }
        write_be64(dp, total_bytes);
        dp += 8;
        write_be32(dp, static_cast<uint32_t>(hash_count_));
        dp += 4;
        write_be32(dp, static_cast<uint32_t>(kHashTypeMurmur3));
        dp += 4;
        write_be64(dp, static_cast<uint64_t>(total_keys_));
        dp += 8;
        uint64_t total_max_keys = 0;
        for (uint32_t c : chunk_max_keys_)
        {
            total_max_keys += c;
        }
        write_be64(dp, total_max_keys);
        dp += 8;
        write_be32(dp, static_cast<uint32_t>(chunk_offsets_.size()));
        dp += 4;
        ...
        for (size_t i = 0; i < chunk_offsets_.size(); ++i)
        {
            ...
            const int key_len_size
                = encode_writable_vint(key_len_buf, static_cast<int64_t>(chunk_first_keys_[i].size()));
            write_be64(dp, static_cast<uint64_t>(chunk_offsets_[i]));
            dp += 8;
            write_be32(dp, chunk_on_disk_sizes_[i]);
            dp += 4;
        ...
        if (compressor && compressor->type() != Compression::None)
        {
            ...
            size_t comp_len
                = compressor->compress({raw_data.data(), raw_data.size()}, compressed.data(), compressed.size());
            if (comp_len > 0)
            {
                compressed.resize(comp_len);
                payload = {compressed.data(), compressed.size()};
        ...
        std::memcpy(p, kBloomMetaMagic.data(), 8);
        p += 8;
        write_be32(p, static_cast<uint32_t>(payload.size() + checksum_buf.size()));
        p += 4;
        write_be32(p, static_cast<uint32_t>(data_size));
        p += 4; // uncompressed original size
        write_be64(p, static_cast<uint64_t>(prev_block_offset));
        p += 8;
        *p++ = kChecksumTypeCRC32C;
        ...
        if (result)
        {
            result->bloom_meta_uncompressed_bytes_with_headers
                = static_cast<uint64_t>(kBlockHeaderSize) + static_cast<uint64_t>(raw_data.size());
        }
    }
