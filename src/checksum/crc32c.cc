...
static uint32_t crc32c_hw(uint32_t crc, const uint8_t* data, size_t len) noexcept
{
    ...
    // _mm_crc32_u64 requires a 64-bit target; on 32-bit we skip the 8-byte
    // loop.
...
size_t
compute_hfile_checksums(const uint8_t* data, size_t data_len, uint32_t bytes_per_chunk, uint8_t* out_checksums) noexcept
{
    size_t out_off = 0;
    size_t offset = 0;
...
size_t compute_hfile_checksums_split(std::span<const uint8_t> first,
