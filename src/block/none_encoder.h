...
{
public:
    explicit NoneEncoder(size_t block_size) : block_size_{block_size}
    {
    ...
    memory::AlignedByteBuffer take_finished_buffer(memory::AlignedByteBuffer replacement) noexcept override
    {
    ...
    size_t required_buffer_storage(size_t encoded_size) const noexcept override
    {
        ...
        const size_t required = encoded_size > max_size - used_ ? max_size : used_ + encoded_size;
        if (required <= buffer_.size())
        {
            return buffer_.size();
        }
        const size_t doubled = buffer_.size() > max_size / 2 ? max_size : buffer_.size() * 2;
    ...
    uint32_t num_kvs() const override
    {
        return num_kvs_;
    }
...
private:
    static size_t initial_buffer_size(size_t block_size) noexcept
    {
        ...
        return block_size > std::numeric_limits<size_t>::max() - kSlack ? block_size : block_size + kSlack;
    }
