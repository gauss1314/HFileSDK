...
public:
    ...
    explicit AtomicFileWriter(const std::string& final_path, size_t buffer_size = size_t{4} * 1024 * 1024);
    ...
    /// Finalise: flush, fsync temp file, rename to final_path, fsync
    /// directories. After a successful commit() the temp file no longer exists.
    Status commit();
    ...
    const std::string& final_path() const noexcept
    {
        return final_path_;
    }
    const std::string& temp_path() const noexcept
    {
        return temp_path_;
    }
