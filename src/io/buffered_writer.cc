...
BufferedFileWriter::BufferedFileWriter(const std::string& path, size_t buffer_size) : buf_cap_{buffer_size}
{
    ...
    if (!file_)
    {
        throw std::runtime_error(std::string("BufferedFileWriter: cannot open '") + path
                                 + "': " + std::strerror(errno));
    }
...
Status BufferedFileWriter::drain()
{
    ...
    {
        return Status::IoError(std::string("fwrite failed: ") + std::strerror(errno));
    }
...
Status BufferedFileWriter::flush()
{
    ...
    {
        return Status::IoError(std::string("fflush failed: ") + std::strerror(errno));
    }

    // Best-effort fsync (POSIX) / no-op (Windows)
    if (HFILE_FSYNC(file_) != 0)
    {
        return Status::IoError(std::string("fsync failed: ") + std::strerror(errno));
    }
...
Status BufferedFileWriter::close()
{
    ...
    if (file_)
    {
        ...
        {
            return Status::IoError(std::string("fclose failed: ") + std::strerror(errno));
        }
        file_ = nullptr;
    }
    if (!drain_status.ok())
...
std::unique_ptr<BlockWriter> BlockWriter::open_file(const std::string& path, size_t buffer_size)
{
