...
IoUringWriter::IoUringWriter(const std::string& path, size_t buffer_size, unsigned ring_depth)
    : ring_depth_{ring_depth}, buf_cap_{buffer_size}
{
    ...
    fd_ = ::open(path.c_str(), O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC | O_DIRECT, 0644);
    if (fd_ < 0)
    {
...
Status IoUringWriter::submit_active()
{
    ...
    if (!sqe)
    {
        return Status::Internal("IoUringWriter: no SQE slot available");
    }

    const int64_t write_offset = pos_ - static_cast<int64_t>(buf_used_[cur_]);
    io_uring_prep_write(
        sqe, fd_, buf_[cur_].data(), static_cast<unsigned>(buf_used_[cur_]), static_cast<uint64_t>(write_offset));
    // Tag with buffer index so the completion handler can verify
    io_uring_sqe_set_data64(sqe, static_cast<uint64_t>(cur_));

    if (io_uring_submit(&ring_) < 0)
    {
        return Status::IoError(std::string("io_uring_submit failed: ") + std::strerror(errno));
    }
    ++in_flight_;
    buf_used_[cur_] = 0;
...
Status IoUringWriter::drain_completions(bool wait_all)
{
    while (in_flight_ > 0)
    {
        ...
        int r = wait_all ? io_uring_wait_cqe(&ring_, &cqe) : io_uring_peek_cqe(&ring_, &cqe);
        if (r == -EAGAIN)
        {
            break;
        }
        if (r < 0)
        {
            return Status::IoError(std::string("io_uring CQE error: ") + std::strerror(-r));
        }
        if (cqe->res < 0)
        {
            return Status::IoError(std::string("async write failed: ") + std::strerror(-cqe->res));
        }
