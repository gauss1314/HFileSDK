...
/// Buffered file writer — cross-platform implementation using standard C
/// FILE*.
///
...
///   Windows (MSYS2 Clang) — FILE* backed by the active C runtime handle;
///   supported.
class BufferedFileWriter final : public BlockWriter
{
public:
    explicit BufferedFileWriter(const std::string& path, size_t buffer_size = size_t{4} * 1024 * 1024);
