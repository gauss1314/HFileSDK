...
inline Status parse_json_config(std::string_view json, JsonConfigObject* out)
{
    ...
    if (pos < json.size() && json[pos] == '}')
    {
        ++pos;
        skip_ws(json, &pos);
        return pos == json.size() ? Status::OK() : Status::InvalidArg("JSON parse error: trailing characters");
    }
    ...
    while (pos < json.size())
    {
        ...
        else if (pos < json.size() && json[pos] == '[')
        {
            ...
            while (pos < json.size() && json[pos] != ']')
            {
                if (json[pos] != '"')
                {
                    return Status::InvalidArg("JSON parse error: array elements must be strings");
                }
                std::string item;
                ...
                {
                    return Status::InvalidArg("JSON parse error: expected ',' or ']' in array");
                }
            }
            if (pos >= json.size())
            {
...
inline std::optional<std::string> config_string(const JsonConfigObject& obj, const std::string& key)
{
...
inline std::optional<int64_t> config_int(const JsonConfigObject& obj, const std::string& key)
{
    auto it = obj.find(key);
    if (it == obj.end())
    {
...
/// Return the string array value for `key`, or nullopt if not present / not an
/// array.
inline std::optional<std::vector<std::string>> config_string_array(const JsonConfigObject& obj, const std::string& key)
{
    auto it = obj.find(key);
    if (it == obj.end())
    {
