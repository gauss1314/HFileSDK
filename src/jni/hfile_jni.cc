...
static std::string result_to_json(const ConvertResult& r)
{
    ...
    oss << "{"
        << "\"error_code\":" << r.error_code << ',' << "\"error_message\":\"" << json_escape(r.error_message) << "\","
        ...
        << "\"numeric_sort_fast_path_mode\":\"" << numeric_sort_fast_path_mode_name(r.numeric_sort_fast_path_mode)
        << "\","
...
static jlong create_instance_state()
{
    ...
    for (;;)
    {
        ...
        if (g_sessions.emplace(handle, state).second)
        {
            return handle;
        }
    }
...
static InstanceStatePtr get_or_create_legacy_instance_state(JNIEnv* env, jobject obj)
{
    ...
    while (it != g_legacy_sessions.end())
    {
        ...
        if (env->IsSameObject(it->weak_ref, obj))
        {
            return it->state;
        }
        ++it;
...
static void set_instance_result(const InstanceStatePtr& state, const hfile::ConvertResult& result)
{
    if (!state)
    {
        return;
...
{
    return hfile::jni::make_error_result(hfile::ErrorCode::INTERNAL_ERROR,
...
extern "C"
{
    ...
    JNIEXPORT jlong JNICALL Java_com_hfile_HFileSDK_nativeCreateSession(JNIEnv*, jclass)
    {
        try
        {
    ...
    JNIEXPORT void JNICALL Java_com_hfile_HFileSDK_nativeDestroySession(JNIEnv*, jclass, jlong handle)
    {
        try
        {
        ...
        {
            // Cleaner callbacks must never surface a C++ exception into the
            // JVM.
                                   ...
                                   jstring j_row_key_rule)
    {
        ...
        try
        {
            ...
            {
                auto r
                    = make_error_result(s.code() == hfile::Status::Code::InvalidArg ? hfile::ErrorCode::INVALID_ARGUMENT
            ...
            if (!s.ok())
            {
                auto r = make_error_result(hfile::ErrorCode::INTERNAL_ERROR, "tableName: " + s.message());
                set_instance_result(state, r);
                return r.error_code;
            }
            ...
            if (!s.ok())
            {
                auto r = make_error_result(hfile::ErrorCode::INTERNAL_ERROR, "rowKeyRule: " + s.message());
                set_instance_result(state, r);
                return r.error_code;
            }
            ...
            // If configure() set a column_family, propagate it into
            // ConvertOptions. Otherwise leave ConvertOptions::column_family at
            // its default ("cf").
        ...
        catch (...)
        {
            ...
            auto r = hfile::jni::make_error_result(hfile::ErrorCode::INTERNAL_ERROR, "Internal C++ exception: unknown");
            set_instance_result(state, r);
            return r.error_code;
        }
    }

    JNIEXPORT jint JNICALL Java_com_hfile_HFileSDK_nativeConvert(JNIEnv* env,
                                                                 jclass,
                                                                 jlong handle,
                                                                 jstring j_arrow_path,
                                                                 jstring j_hfile_path,
                                                                 ...
                                                                 jstring j_row_key_rule)
    {
        try
        {
            return convert_with_state(
                env, find_instance_state(handle), j_arrow_path, j_hfile_path, j_table_name, j_row_key_rule);
        }
        catch (...)
        {
            return hfile::ErrorCode::INTERNAL_ERROR;
     ...
     */
    static jstring get_last_result_with_state(JNIEnv* env, const InstanceStatePtr& state)
    {
        try
        {
            if (!state)
            {
                const std::string invalid_json = hfile::jni::result_to_json(invalid_session_result());
    ...
    JNIEXPORT jstring JNICALL Java_com_hfile_HFileSDK_nativeGetLastResult(JNIEnv* env, jclass, jlong handle)
    {
        try
        {
    ...
    {
        hfile::WriterOptions writer_options;
        hfile::NumericSortFastPathMode numeric_sort_fast_path;
        int64_t default_timestamp_ms;
        std::vector<std::string> excluded_columns;
        std::vector<std::string> excluded_column_prefixes;
    ...
    static hfile::Status validate_config_keys(const hfile::jni::JsonConfigObject& config)
    {
        static const std::unordered_set<std::string> kAllowedKeys = {"compression",
                                                                     "compression_level",
                                                                     "block_size",
                                                                     "column_family",
                                                                     "data_block_encoding",
        ...
        for (const auto& [key, value] : config)
        {
            ...
            {
                return hfile::Status::InvalidArg("Unsupported config key: " + key);
            }
        }
        return hfile::Status::OK();
    }

    static hfile::Status apply_format_config(const hfile::jni::JsonConfigObject& config, hfile::WriterOptions* options)
    {
        if (auto compression = hfile::jni::config_string(config, "compression"))
        {
            const std::string normalized = hfile::jni::ascii_lower(*compression);
            if (normalized == "none")
            {
                options->compression = hfile::Compression::None;
            }
            else if (normalized == "gzip" || normalized == "gz")
            {
                options->compression = hfile::Compression::GZip;
            }
            else
            {
                return hfile::Status::InvalidArg("Only compression=NONE, GZ, or gzip is supported");
            }
        }

        if (auto level = hfile::jni::config_int(config, "compression_level"))
        {
            if (*level < 0 || *level > 9)
            {
                return hfile::Status::InvalidArg("compression_level must be 0-9");
            }
            options->compression_level = static_cast<int>(*level);
        }
        if (auto block_size = hfile::jni::config_int(config, "block_size"))
        {
            if (*block_size <= 0)
            {
                return hfile::Status::InvalidArg("block_size must be > 0");
            }
            options->block_size = static_cast<size_t>(*block_size);
        }
        if (auto family = hfile::jni::config_string(config, "column_family"))
        {
            if (family->empty())
            {
                return hfile::Status::InvalidArg("column_family must not be empty");
            }
            options->column_family = *family;
        }
        if (auto encoding = hfile::jni::config_string(config, "data_block_encoding"))
        {
            if (*encoding != "NONE")
            {
                return hfile::Status::InvalidArg("Only data_block_encoding=NONE is supported");
            }
            options->data_block_encoding = hfile::Encoding::None;
        }

        if (auto policy = hfile::jni::config_string(config, "fsync_policy"))
        {
            if (*policy == "safe")
            {
                options->fsync_policy = hfile::FsyncPolicy::Safe;
            }
            else if (*policy == "fast")
            {
                options->fsync_policy = hfile::FsyncPolicy::Fast;
            }
            else if (*policy == "paranoid")
            {
                options->fsync_policy = hfile::FsyncPolicy::Paranoid;
            }
            else
            {
                return hfile::Status::InvalidArg("Invalid fsync_policy: " + *policy);
            }
        }
        if (auto policy = hfile::jni::config_string(config, "error_policy"))
        {
            if (*policy == "strict")
            {
                options->error_policy = hfile::ErrorPolicy::Strict;
            }
            else if (*policy == "skip_row")
            {
                options->error_policy = hfile::ErrorPolicy::SkipRow;
            }
            else if (*policy == "skip_batch")
            {
                options->error_policy = hfile::ErrorPolicy::SkipBatch;
            }
            else
            {
                return hfile::Status::InvalidArg("Invalid error_policy: " + *policy);
            }
        }
        if (auto bloom_type = hfile::jni::config_string(config, "bloom_type"))
        {
            if (*bloom_type == "none")
            {
                options->bloom_type = hfile::BloomType::None;
            }
            else if (*bloom_type == "row")
            {
                options->bloom_type = hfile::BloomType::Row;
            }
            else if (*bloom_type == "rowcol")
            {
                options->bloom_type = hfile::BloomType::RowCol;
            }
            else
            {
                return hfile::Status::InvalidArg("Invalid bloom_type: " + *bloom_type);
            }
        }
        if (auto include_mvcc = hfile::jni::config_int(config, "include_mvcc"))
        {
            if (*include_mvcc != 0 && *include_mvcc != 1)
            {
                return hfile::Status::InvalidArg("include_mvcc must be 0 or 1");
            }
            options->include_mvcc = *include_mvcc != 0;
        }
        return hfile::Status::OK();
    }

    static hfile::Status apply_resource_config(const hfile::jni::JsonConfigObject& config, ConfigurationDraft* draft)
    {
        if (auto max_memory = hfile::jni::config_int(config, "max_memory_bytes"))
        {
            if (*max_memory < 0)
            {
                return hfile::Status::InvalidArg("max_memory_bytes must be >= 0");
            }
            draft->writer_options.max_memory_bytes = static_cast<size_t>(*max_memory);
        }
        if (auto thread_count = hfile::jni::config_int(config, "compression_threads"))
        {
            if (*thread_count < 0 || static_cast<uint64_t>(*thread_count) > std::numeric_limits<uint32_t>::max())
            {
                return hfile::Status::InvalidArg("compression_threads must be 0-4294967295");
            }
            draft->writer_options.compression_threads = static_cast<uint32_t>(*thread_count);
        }
        if (auto queue_depth = hfile::jni::config_int(config, "compression_queue_depth"))
        {
            if (*queue_depth < 0 || *queue_depth > 4096)
            {
                return hfile::Status::InvalidArg("compression_queue_depth must be 0-4096");
            }
            draft->writer_options.compression_queue_depth = static_cast<uint32_t>(*queue_depth);
        }

        if (auto fast_path = hfile::jni::config_string(config, "numeric_sort_fast_path"))
        {
            const std::string normalized = hfile::jni::ascii_lower(*fast_path);
            if (normalized == "auto")
            {
                draft->numeric_sort_fast_path = hfile::NumericSortFastPathMode::Auto;
            }
            else if (normalized == "on")
            {
                draft->numeric_sort_fast_path = hfile::NumericSortFastPathMode::On;
            }
            else if (normalized == "off")
            {
                draft->numeric_sort_fast_path = hfile::NumericSortFastPathMode::Off;
            }
            else
            {
                return hfile::Status::InvalidArg("numeric_sort_fast_path must be one of: auto|on|off");
            }
        }
        if (auto timestamp = hfile::jni::config_int(config, "default_timestamp_ms"))
        {
            if (*timestamp < 0)
            {
                return hfile::Status::InvalidArg("default_timestamp_ms must be >= 0");
            }
            draft->default_timestamp_ms = *timestamp;
        }
        if (auto columns = hfile::jni::config_string_array(config, "excluded_columns"))
        {
            draft->excluded_columns = std::move(*columns);
        }
        if (auto prefixes = hfile::jni::config_string_array(config, "excluded_column_prefixes"))
        {
            draft->excluded_column_prefixes = std::move(*prefixes);
        }
        return hfile::Status::OK();
    }
     ...
     * configJson:
     * {"compression":"GZ","block_size":65536,"column_family":"cf",...}
     */
    static jint configure_with_state(JNIEnv* env, const InstanceStatePtr& state, jstring j_config)
    {
        if (!state)
        {
            return hfile::ErrorCode::INTERNAL_ERROR;
        ...
        try
        {
            ...
            std::lock_guard<std::mutex> lock(state->mutex);
            ConfigurationDraft draft{state->writer_opts,
                                     state->numeric_sort_fast_path,
                                     state->default_timestamp_ms,
                                     state->excluded_columns,
                                     state->excluded_column_prefixes};
            auto fail_config = [&](std::string message)
            {
                state->last_result
                    = hfile::jni::make_error_result(hfile::ErrorCode::INVALID_ARGUMENT, std::move(message));
            ...
            {
                state->last_result
                    = hfile::jni::make_error_result(hfile::ErrorCode::INVALID_ARGUMENT, parse_status.message());
                state->last_result_json = hfile::jni::result_to_json(state->last_result);
                return state->last_result.error_code;
            }

            auto validation_status = validate_config_keys(cfg);
            if (validation_status.ok())
            {
                validation_status = apply_format_config(cfg, &draft.writer_options);
            }
            if (validation_status.ok())
            {
                validation_status = apply_resource_config(cfg, &draft);
            }
            if (!validation_status.ok())
            {
                return fail_config(validation_status.message());
            }

            state->writer_opts = std::move(draft.writer_options);
            state->numeric_sort_fast_path = draft.numeric_sort_fast_path;
            state->default_timestamp_ms = draft.default_timestamp_ms;
            state->excluded_columns = std::move(draft.excluded_columns);
            state->excluded_column_prefixes = std::move(draft.excluded_column_prefixes);
            state->last_result = {};
            state->last_result_json = hfile::jni::result_to_json(state->last_result);
            return 0;
        }
    ...
    JNIEXPORT jint JNICALL Java_com_hfile_HFileSDK_nativeConfigure(JNIEnv* env, jclass, jlong handle, jstring j_config)
    {
        try
        {
    ...
    JNIEXPORT jboolean JNICALL Java_com_hfile_HFileSDK_nativeSessionExistsForTesting(JNIEnv*, jclass, jlong handle)
    {
        try
        {
    ...
    // ---- These exact symbols keep older
    // already-compiled Java artifacts loadable. The new class calls only
    // native*Session/native* methods above, so its hot path never scans the
    // compatibility weak-reference table.
    JNIEXPORT jint JNICALL Java_com_hfile_HFileSDK_convert(JNIEnv* env,
                                                           jobject obj,
                                                           jstring j_arrow_path,
                                                           jstring j_hfile_path,
                                                           jstring j_table_name,
    ...
    JNIEXPORT jstring JNICALL Java_com_hfile_HFileSDK_getLastResult(JNIEnv* env, jobject obj)
    {
        try
        {
            return get_last_result_with_state(env, get_or_create_legacy_instance_state(env, obj));
        }
        catch (...)
        {
            return env->NewStringUTF("{}");
    ...
    JNIEXPORT jint JNICALL Java_com_hfile_HFileSDK_configure(JNIEnv* env, jobject obj, jstring j_config)
    {
        try
        {
            return configure_with_state(env, get_or_create_legacy_instance_state(env, obj), j_config);
        }
        catch (...)
        {
            return hfile::ErrorCode::INTERNAL_ERROR;
