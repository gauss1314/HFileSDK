...
template <typename Function> bool invoke_best_effort(Function&& function) noexcept
{
    try
    {
...
template <typename Value, typename Function> Value invoke_or(Value fallback, Function&& function) noexcept
{
    try
    {
...
public:
    ...
    {
        (void)metrics_detail::invoke_best_effort(
            [&]()
            {
    ...
    {
        return metrics_detail::invoke_or<int64_t>(0,
                                                  [&]()
                                                  {
                                                      std::lock_guard<std::mutex> lock{mu_};
                                                      auto iterator = counters_.find(name);
                                                      return iterator != counters_.end()
    ...
    {
        return metrics_detail::invoke_or<double>(0.0,
                                                 [&]()
                                                 {
                                                     std::lock_guard<std::mutex> lock{mu_};
                                                     auto iterator = gauges_.find(name);
                                                     return iterator != gauges_.end() ? iterator->second : 0.0;
                                                 });
    }
    ...
    {
        return metrics_detail::invoke_or<HistogramSnapshot>(
            {},
            [&]() -> HistogramSnapshot
            {
                std::lock_guard<std::mutex> lock{mu_};
                auto iterator = histograms_.find(name);
                if (iterator == histograms_.end() || iterator->second.empty())
                {
                    return {};
                }
                std::vector<double> sorted = iterator->second;
                std::sort(sorted.begin(), sorted.end());
                HistogramSnapshot snapshot;
                snapshot.count = static_cast<int64_t>(sorted.size());
                ...
                {
                    const double element_count = static_cast<double>(sorted.size());
                    size_t index = static_cast<size_t>(std::ceil(fraction * element_count) - 1);
    ...
    void set_report_callback(ReportCallback cb, std::chrono::seconds interval)
    {
        stop_report_thread();
...
private:
    ...
    static HistogramSnapshot histogram_unlocked(const std::vector<double>& vals)
    {
        ...
        auto pct = [&](double p)
        {
            const double element_count = static_cast<double>(s.size());
            size_t idx = static_cast<size_t>(std::ceil(p * element_count) - 1);
...
struct ScopedTimer
{
    ScopedTimer(MetricsRegistry& reg, std::string name)
        : reg_{reg}, name_{std::move(name)}, start_{std::chrono::steady_clock::now()}
    {
    }
