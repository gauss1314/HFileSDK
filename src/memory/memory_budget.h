...
public:
    ...
    explicit MemoryBudget(size_t max_bytes = kUnlimited) noexcept : max_bytes_{max_bytes}, used_{0}, peak_{0}
    {
    }
    ...
    Status reserve(size_t bytes)
    {
        ...
        for (;;)
        {
            ...
            {
                return Status::InvalidArg("MemoryBudget: limit exceeded (" + std::to_string(max_bytes_ / 1024 / 1024)
                                          + " MB)");
            }
            next = current + bytes;
            if (used_.compare_exchange_weak(current, next, std::memory_order_relaxed, std::memory_order_relaxed))
            {
                break;
            }
        }
        ...
        while (next > p && !peak_.compare_exchange_weak(p, next, std::memory_order_relaxed, std::memory_order_relaxed))
        {
        }
        return Status::OK();
    }
    ...
    struct Guard
    {
        Guard(MemoryBudget& b, size_t bytes) : budget_{b}, bytes_{0}
        {
            if (b.reserve(bytes).ok())
            {
                bytes_ = bytes;
            }
        }
