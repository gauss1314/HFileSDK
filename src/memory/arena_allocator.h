...
{
public:
    static constexpr size_t kDefaultChunkSize = size_t{256} * 1024; // 256 KB

    explicit ArenaAllocator(size_t chunk_size = kDefaultChunkSize) : chunk_size_{chunk_size}
    {
        new_chunk();
    }
...
private:
    void new_chunk(size_t size = 0)
    {
        ...
        auto* chunk = static_cast<uint8_t*>(::operator new[](size, std::align_val_t{64}));
