...
///   - Releasing a pointer not belonging to this pool is detected (assert
///   fires).
class BlockPool
{
public:
    ...
    explicit BlockPool(size_t buffer_size, size_t pool_count) : buffer_size_{buffer_size}, pool_count_{pool_count}
    {
        raw_.reserve(pool_count);
        free_.reserve(pool_count);
        for (size_t i = 0; i < pool_count; ++i)
        {
            auto* buf = static_cast<uint8_t*>(::operator new[](buffer_size, std::align_val_t{64}));
            raw_.push_back(buf);
            free_.push_back(buf);
    ...
    void release(uint8_t* buf) noexcept
    {
        ...
        assert(owned_.count(buf) > 0 && "BlockPool::release: pointer not owned by this pool");
        // Detect double-release
        assert(in_use_.count(buf) > 0 && "BlockPool::release: double-release detected");
        in_use_.erase(buf);
#endif
