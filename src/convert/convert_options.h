...
inline constexpr const char* numeric_sort_fast_path_mode_name(NumericSortFastPathMode mode) noexcept
{
    switch (mode)
    {
        case NumericSortFastPathMode::Auto:
            return "auto";
        case NumericSortFastPathMode::On:
            return "on";
        case NumericSortFastPathMode::Off:
            return "off";
    }
    return "auto";
}
...
struct ConvertOptions
{
    ...
    // -- Column filtering ----
    /// Column names to exclude from HBase KV output (exact match,
    /// case-sensitive). These columns are NOT included in the pipe-delimited
    /// HBase cell value. Does NOT affect row key construction — row key
    /// segments reference columns by index (from the original Arrow schema)
    ...
    /// whose name starts with "_hoodie").  Takes effect after exact-name
    /// exclusions. Matching is prefix-only, case-sensitive.
    ///
    /// Example: {"_hoodie"}  →  drops all five Hudi metadata columns
    /// automatically
...
struct ConvertResult
{
    ...
    /// Number of HBase row keys that were produced by more than one Arrow
    /// source row (i.e. rowKeyRule collisions).  Each such group emits one WARN
    /// log line. Non-zero here means your rowKeyRule is not injective for this
    /// dataset; review the rule or add a uniqueness-guaranteeing segment (e.g.
    /// $RND$).
