...
public:
    ...
    {
        set_bytes(std::string(fileinfo::kComparator), {reinterpret_cast<const uint8_t*>(cmp.data()), cmp.size()});
    }
    ...
    void set_bloom_filter_type(BloomType type)
    {
        ...
        switch (type)
        {
            ...
            case BloomType::Row:
                name = "ROW";
                break;
            case BloomType::RowCol:
                name = "ROWCOL";
                break;
        }
        set_bytes(std::string(fileinfo::kBloomFilterType), {reinterpret_cast<const uint8_t*>(name), strlen(name)});
    }
    ...
    void set_create_time(int64_t ts_ms = -1)
    {
        if (ts_ms < 0)
        {
            ts_ms = static_cast<int64_t>(std::chrono::duration_cast<std::chrono::milliseconds>(
                                             std::chrono::system_clock::now().time_since_epoch())
                                             .count());
        }
        uint8_t buf[8];
        write_be64(buf, static_cast<uint64_t>(ts_ms));
    ...
    Status validate_required_fields() const
    {
        ...
        {
            return Status::InvalidArg("FileInfo missing mandatory field: " + std::string(fileinfo::kTagsCompressed));
        }
        return Status::OK();
    }
