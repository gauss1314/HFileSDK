...
void BlockIndexWriter::add_entry(std::span<const uint8_t> first_key, int64_t offset, int32_t data_size)
{
    IndexEntry e;
...
void BlockIndexWriter::write_entry(const IndexEntry& e, std::vector<uint8_t>& buf)
{
    ...
    int key_len_size = encode_writable_vint(key_len_buf, static_cast<int64_t>(e.first_key.size()));
...
size_t BlockIndexWriter::write_intermediate_block(std::span<const IndexEntry* const> entries,
                                                  ...
                                                  std::vector<uint8_t>& buf)
{
    ...
    std::memcpy(p, kIntermedIdxMagic.data(), 8);
    p += 8;
    write_be32(p, static_cast<uint32_t>(payload + checksum_buf.size()));
    p += 4;
    write_be32(p, static_cast<uint32_t>(payload));
    p += 4; // uncompressedSz
    ...
    *p++ = kChecksumTypeCRC32C;
    ...
    checksum::compute_hfile_checksums(
        buf.data() + start, kBlockHeaderSize + payload, kBytesPerChecksum, checksum_buf.data());
    std::memcpy(buf.data() + start + kBlockHeaderSize + payload, checksum_buf.data(), checksum_buf.size());
                                          ...
                                          std::vector<uint8_t>& root_out)
{
    ...
    while (i < entries_.size())
    {
        ...
        size_t block_bytes
            = write_intermediate_block(std::span<const IndexEntry* const>{chunk_ptrs.data(), chunk_ptrs.size()},
    ...
    result.uncompressed_size
        = static_cast<uint64_t>(intermed_blocks_out.size()) + static_cast<uint64_t>(root_out.size());
    return result;
}
