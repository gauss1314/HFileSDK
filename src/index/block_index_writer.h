...
///
///   // At finish time:
///   std::vector<uint8_t> intermed_buf;   // receives serialised intermediate
///   blocks std::vector<uint8_t> root_buf;       // receives root-level index
///   payload int64_t intermed_start = writer->position(); auto result =
///   index_writer_.finish(intermed_start, intermed_buf, root_buf); if
///   (!intermed_buf.empty())
...
public:
    ...
    explicit BlockIndexWriter(size_t max_entries_per_block = 128) : max_per_block_{max_entries_per_block}
    {
    }

    /// Record a data block at file position `offset`.
    void add_entry(std::span<const uint8_t> first_key, int64_t offset, int32_t data_size);
    ...
    /// @param root_out               Receives the root index payload (the
    /// caller
    ///                               wraps this in a block with
    ///                               kRootIndexMagic).
    IndexWriteResult
    finish(int64_t intermed_start_offset, std::vector<uint8_t>& intermed_blocks_out, std::vector<uint8_t>& root_out);
