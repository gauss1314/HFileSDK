...
static std::vector<std::string_view> split_sv(std::string_view s, char delim, int max_count = -1)
{
...
{
    return name == "$RND$" || name == "RND$" || name == "$RND" || iequal(name, "RANDOM") || iequal(name, "RANDOM_COL");
}
...
static void append_segment_value(std::string* out, const RowKeySegment& seg, std::string_view value)
{
...
static Status encode_long_or_short(const RowKeySegment& seg, std::string_view value, std::string* out)
{
    ...
    if (seg.encode_kind == RowKeySegment::EncodeKind::Int64Base64)
    {
        ...
        {
            return Status::InvalidArg("rowKeyRule encoded segment '" + seg.name + "' cannot parse int64 value '"
                                      + encoded + "'");
        }
    ...
    {
        return Status::InvalidArg("rowKeyRule encoded segment '" + seg.name + "' cannot parse int16 value '" + encoded
                                  + "'");
    }
...
std::pair<RowKeyBuilder, Status> RowKeyBuilder::compile(const std::string& rule)
{
    ...
    for (const auto& seg_sv : seg_strs)
    {
        if (seg_sv.empty())
        {
            continue; // skip empty segments (trailing #)
        }
        ...
        if (fields.size() < 4)
        {
            return {std::move(builder),
                    Status::InvalidArg("rowKeyRule segment too short (need ≥4 fields): " + std::string(seg_sv))};
        }
        ...
        {
            int idx = 0;
            auto [ptr, ec] = std::from_chars(fields[1].data(), fields[1].data() + fields[1].size(), idx);
            if (ec != std::errc{} || ptr != fields[1].data() + fields[1].size())
            {
                return {std::move(builder),
                        Status::InvalidArg("rowKeyRule: invalid index '" + std::string(fields[1]) + "'")};
            }
            if (idx < 0)
            {
                return {std::move(builder), Status::InvalidArg("rowKeyRule: index must be >= 0")};
            }
            seg.col_index = idx;
        }

        // isReverse — field[2]: "true" / "false"
        if (iequal(fields[2], "true"))
        {
            seg.reverse = true;
        }
        else if (iequal(fields[2], "false"))
        {
            seg.reverse = false;
        }
        else
        {
            return {std::move(builder),
                    Status::InvalidArg("rowKeyRule: isReverse must be true or false, got '" + std::string(fields[2])
                                       + "'")};
        }
        ...
        {
            int pl = 0;
            auto [ptr, ec] = std::from_chars(fields[3].data(), fields[3].data() + fields[3].size(), pl);
            if (ec != std::errc{} || ptr != fields[3].data() + fields[3].size())
            {
                return {std::move(builder),
                        Status::InvalidArg("rowKeyRule: invalid padLen '" + std::string(fields[3]) + "'")};
            }
            if (pl < 0)
            {
                return {std::move(builder), Status::InvalidArg("rowKeyRule: padLen must be >= 0")};
            }
            seg.pad_len = pl;
        }
        ...
        if (fields.size() >= 5)
        {
            if (iequal(fields[4], "RIGHT"))
            {
                seg.pad_right = true;
            }
            else if (iequal(fields[4], "LEFT"))
            {
                seg.pad_right = false;
            }
            else
            {
                return {std::move(builder),
                        Status::InvalidArg("rowKeyRule: padMode must be LEFT or RIGHT, got '" + std::string(fields[4])
                                           + "'")};
            }
        }
        ...
        if ((seg.type == RowKeySegment::Type::ColumnRef || seg.type == RowKeySegment::Type::EncodedColumn)
                                                 ...
                                                 std::string* out)
{
    ...
    for (size_t index = start_segment_index; index < segments_.size(); ++index)
    {
        ...
        if (seg.type != RowKeySegment::Type::Fill)
        {
            ...
            {
                return Status::InvalidArg("rowKeyRule references missing field index " + std::to_string(seg.col_index));
            }
...
std::vector<std::string_view> split_row_value(std::string_view row_value, int max_count)
{
