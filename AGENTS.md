...
本文件为 Codex 在 HFileSDK 项目中工作时提供指引。
...
- **依赖**：Apache Arrow C++ **16.0.0**、protobuf 3.21+、zlib/zlib-ng、spdlog 1.12+、JDK 21（仅需 `jni.h`）、Google Test、jemalloc（可选）
- **Java 工具**：Java **21**（`tools/`、`java/` 均使用 Java 21）
- **编译参数（Release）**：`-O3 -march=native -flto`
- **平台**：Linux x86-64 完整支持；macOS 完整支持（无 io_uring）；Windows 通过 MSYS2 + clang/clang++ + `.bat/.sh` 脚本入口支持核心功能（无 io_uring/HDFS）
...
# 格式化代码
find include src test tools -name '*.cc' -o -name '*.h' | xargs clang-format -i
