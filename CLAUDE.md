...
- **内存可控**：Arrow 文件两遍扫描，全量 batch 保留在内存用于排序；`MemoryBudget` 可设置内存上限（默认无限制）。AutoSort 大文件需要 1–3× 文件大小的内存。
...
# 运行单个测试
./build/test/test_data_block_encoder
./build/test/test_row_key_builder
