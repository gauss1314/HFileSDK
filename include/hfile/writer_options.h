...
enum class ErrorPolicy : uint8_t
{
    ...
    /// Skip the entire RecordBatch if any row in it fails, continue with next
    /// batch.
    SkipBatch = 2,
};
...
struct WriterOptions
{
    ...
    /// Maximum memory the writer may consume across buffers / AutoSort staging
    /// (0 = unlimited).
    ...
    /// Minimum free disk space required; writing stops if below this (0 =
    /// disabled).
    ...
    /// This is retained for API compatibility with the single-file conversion
    /// path.
