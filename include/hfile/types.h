...
/// (more than 10 continuation bytes — a uint64_t can hold at most
/// 10 × 7 bits).
...
{
    const int first = src[0] <= 127 ? static_cast<int>(src[0]) : static_cast<int>(src[0]) - 256;
    if (first >= -112)
    {
        out = first;
        return 1;
...
// --- Block Header size
// ---- 8 (magic) + 4
// (compSz) + 4 (uncompSz) + 8 (prevBlockOffset)
...
// --- Default block size ----
inline constexpr size_t kDefaultBlockSize = size_t{64} * 1024; // 64 KB
...
// ----
inline constexpr std::string_view kCellComparator = "org.apache.hadoop.hbase.InnerStoreCellComparator";
inline constexpr std::string_view kCellComparatorImpl = "org.apache.hadoop.hbase.CellComparatorImpl";
inline constexpr std::string_view kMetaCellComparator = "org.apache.hadoop.hbase.MetaCellComparator";
inline constexpr std::string_view kLegacyKeyValueComparator = "org.apache.hadoop.hbase.KeyValue$KVComparator";
inline constexpr std::string_view kLegacyMetaKeyValueComparator = "org.apache.hadoop.hbase.KeyValue$MetaComparator";
...
struct KeyValue
{
    ...
    {
        return static_cast<uint32_t>(2 + row.size() + 1 + family.size() + qualifier.size() + 8 + 1);
    }
    ...
    size_t encoded_size() const noexcept
    {
        return 4 + 4             // keyLen + valueLen
               ...
               + (has_memstore_ts ? static_cast<size_t>(writable_vint_size(static_cast<int64_t>(memstore_ts)))
...
{
    // Row
    int r = std::memcmp(a.row.data(), b.row.data(), std::min(a.row.size(), b.row.size()));
    if (r != 0)
    {
        return r;
    }
    ...
    // Family
    r = std::memcmp(a.family.data(), b.family.data(), std::min(a.family.size(), b.family.size()));
    if (r != 0)
    {
        return r;
    }
    ...
    // Qualifier
    r = std::memcmp(a.qualifier.data(), b.qualifier.data(), std::min(a.qualifier.size(), b.qualifier.size()));
    if (r != 0)
    {
        return r;
    }
