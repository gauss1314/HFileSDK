...
## 6. 从“大文件下接近 Java”到“稳定超过 3x”的完整优化演进

这一段是本轮优化最需要向技术评委解释清楚的部分：C++ 在大文件场景下并不是一开始就稳定领先 3 倍以上，而是经历了“先把局部热点做快，再发现真正瓶颈，再把主瓶颈打穿”的过程。

### 6.1 起点问题：先解决“能不能公平比、结果是否可信”

在真正追求吞吐之前，最先解决的不是性能绝对值，而是比较口径和正确性口径：

- 纯 Java 路径先补齐了 Bloom 和必要 FileInfo
- 两条路径统一为：
  - `compression = GZ`
  - `compression level = 1`
  - `encoding = NONE`
- `hfile-bulkload-perf` 改造成“父进程调度 + 独立 worker 子进程执行”
- 父进程对 JNI/C++ 与纯 Java 按实现/轮次串行调度，避免磁盘 IO 互相抢占

这一阶段本身不直接提升吞吐，但它决定了后续所有性能结论是否可信。如果文件格式不一致、压缩配置不一致、或者 JNI 与 Java 共处同一 JVM 中，那么任何“谁更快”的结论都不具备生产评审意义。

### 6.2 第一轮有效优化：先把 Pass1 排序路径做快

最初最直观的优化方向，是 C++ 应该先在 Pass1 排序路径上明显领先 Java，因此先做了两类低风险改动：

- `Pass1` 排序从“更稳但更重”的 `stable_sort` 语义，改成“显式 tie-breaker 的 `std::sort``
  - 比较顺序为：`row_key -> batch_idx -> row_idx`
  - 这样既保留重复 row key 的原始相对顺序，也降低排序成本
- qualifier 顺序按 schema 预计算一次后复用
  - 避免每个 `RecordBatch` 重复做同样的列排序工作

这轮优化之后，大文件样例中 JNI/C++ 的排序阶段已经明显快于纯 Java。例如在一次 `single-500mb` 对比中：

- JNI/C++ `sort = 800 ms`
- 纯 Java `sort = 1626 ms`
- 排序阶段加速约 `2.03x`

这说明 C++ 在数据整理和排序阶段确实具备显著优势。

### 6.3 第一轮优化后的关键发现：排序更快，但总耗时仍然没有被真正拉开

同一轮 `single-500mb` 样例里，虽然排序阶段已经拉开差距，但端到端结果只有：
...
这一步是整个优化路线里最关键的认知转折点。它说明：

- 大文件场景下，真正决定总耗时的不是排序
- 即使排序明显更快，只要 `Pass2` 的 block 写入和 `GZ` 压缩还占主导，总体领先幅度就会被迅速吃掉
- 换句话说，C++ 与 Java 在大文件上“一开始越跑越接近”，根因不是排序优化不够，而是大文件已经进入了“压缩吞吐主导”的阶段

### 6.4 第二轮优化：用热点分析把真正瓶颈钉死

为了避免继续在次要路径上“盲目优化”，后续在 C++ 转换和 writer 热点路径里加入了更细的 profiling，把总耗时拆成：
...
最新热点样例 `single-100mb` 的结果非常明确：

- 不启用压缩流水线时：
  - 总耗时约 `1869 ms`
  - `sort` 约 `73 ms`
  - `Pass2 materialize` 约 `63 ms`
  - `Pass2 append` 约 `1600 ms`
  - 其中 `compress` 约 `1096 ms`
- 启用压缩流水线 `4x8` 后：
  - 总耗时约 `861 ms`
  - `sort` 约 `70 ms`
  - `Pass2 append` 约 `583 ms`
  - `compress` 的聚合 worker 时间虽仍高，但已大幅重叠到关键路径之外

这组数据证明：

- 排序已经不再是大文件主导瓶颈
- value materialize 也不是首要问题
- 真正应该继续优化的是 `GZ` block compression

### 6.5 第三轮优化：把压缩路径上的固定成本先清掉

在明确瓶颈已经转移到压缩之后，先做了两类“纯收益、低风险”的 C++ 写路径优化：

- writer 级 gzip 状态复用
  - 每个 block 都 `deflateInit2/deflateEnd` 的模式被替换成“每个 writer 复用一份 zlib/deflater 状态”
  - 这直接消除了大量 block 级固定开销
- `Encoding::None` 默认路径复用原始 CRC
  - 避免 gzip trailer 生成时重复做 checksum 计算

这一阶段的价值在于，它不是只对某一张表有效，也不是只对某一种 rowkey 有效，而是对所有 `GZ` 工作负载都生效，尤其能明显降低小文件和中等文件场景中的固定成本。

### 6.6 第四轮优化：块压缩流水线并行化，真正把大文件瓶颈打穿

真正把大文件场景从“只领先约 1.3x”拉到“稳定超过 3x”的决定性动作，是可配置的块压缩流水线：

- 数据块压缩转移到后台 worker
- 前台 append / 组块 / 写索引 / Bloom 刷出继续推进
- 通过 `compression_threads` 与 `compression_queue_depth` 控制并行度与在途块深度
- 异步路径必须保持与同步路径字节级一致，包括 Bloom chunk 的刷写顺序

这一步的核心不是“线程越多越好”，而是让压缩工作与前台写路径发生有效重叠，让 `compress` 不再完全占据端到端关键路径。

代表性结果：

- `single-100mb` JNI/C++ 热点样例：
  - 不开压缩流水线约 `1869 ms`
  - 开启 `4x8` 压缩流水线后约 `861 ms`
- 正式矩阵中，`single-500mb` 已从中期样例的 `1.31x`，提升到正式版 `3.85x`
- 本次最新回归中，`single-500mb` 仍保持 `3.78x`

这说明大文件性能领先的形成不是偶然值，而是已经建立在稳定的工程机制之上。

### 6.7 第五轮优化：把排序收益从“单表优化”提升为“规则形态优化”

在压缩主瓶颈打穿之后，排序路径仍继续做了一个重要但更克制的增强：将数值型 rowkey 快速排序路径做成“按规则形态启用”的通用机制，而不是把优化写死在某一张表上。

具体策略是：

- `numeric_sort_fast_path = auto | on | off`
- 只对“rowkey 第一段是零左填充、非 reverse、非右填充的数值/时间戳前缀”启用快速路径
- 若输入不满足规则，`auto` 自动回退，`on` 明确报错

这一步的收益主要体现在两个方面：

- 数值前缀 rowkey 场景下，排序成本进一步下降
- 优化对象从“某张表”扩展成了“一类 rowkey 规则”，避免了单表特化带来的泛化风险

### 6.8 为什么现在能稳定超过 3x，但大文件领先倍数仍然比小文件小

当前结果里，小文件场景如 `single-001mb` 可以达到 `11.08x`，而大文件场景如 `single-500mb` 为 `3.78x`。这并不是大文件优化不成功，而是系统已经进入了新的瓶颈阶段：

- 小文件里，固定成本占比高
  - 一旦去掉 block 级初始化、重复 checksum、同步压缩等待，这类固定成本会被大幅摊薄，因此倍数增长非常明显
- 大文件里，固定成本占比会越来越小
  - 最终主要比拼的是持续 `GZ` 压缩吞吐
  - 因此端到端领先倍数会自然收敛到 `3x~4x` 区间

换句话说，当前“大文件倍数比小文件小”并不是回退，而是因为系统已经从“排序/组织开销主导”转入“压缩吞吐主导”。这恰恰说明前面的排序和写路径优化已经奏效，剩下的空间主要在压缩体系本身。

### 6.9 阶段性总结

从演进过程看，C++ 大文件性能最终能稳定超过 3 倍，不是依赖单一技巧，而是依赖三次明确的认知升级：

- 第一，先确保两条路径格式等价、口径公平，避免“快但不可比”
- 第二，通过中期 `single-500mb` 和后续热点分析，明确真正瓶颈不是排序而是压缩
- 第三，围绕纯 C++ gzip 状态复用、CRC 复用、块压缩流水线并行化，把主瓶颈从端到端关键路径上实质性移开

最终效果是：

- 中期大文件样例：约 `1.31x`
- 正式基线 `single-500mb`：`3.85x`
- 最新回归 `single-500mb`：`3.78x`
- 当前六个标准场景全部满足 `JNI/C++ >= 3x 纯 Java`

## 7. C++ 性能优化技术方案回顾

### 7.1 已选方案：保持 HFile 字节与格式语义稳定
...
### 7.5 已选方案：`Encoding::None` 路径复用原始 CRC
...
### 7.7 已选方案：数值型 rowkey 快速排序路径，但以规则形态驱动而不是单表特化
