...
{
    return std::vector<uint8_t>(reinterpret_cast<const uint8_t*>(s), reinterpret_cast<const uint8_t*>(s) + strlen(s));
}
...
TEST(RegionPartitioner, SingleSplit)
{
    ...
    EXPECT_EQ(p->region_for(key("m")),
              1);                          // "m" == split → region 1 (upper_bound)
    EXPECT_EQ(p->region_for(key("z")), 1); // "z" > "m" → region 1
}

TEST(RegionPartitioner, MultipleSplits)
{
    std::vector<std::vector<uint8_t>> splits = {key("d"), key("h"), key("n"), key("t")};
    auto p = RegionPartitioner::from_splits(splits);
...
TEST(RegionPartitioner, BinaryKeyLookup)
{
    ...
    uint8_t k0[4];
    hfile::write_be32(k0, 0);
    EXPECT_EQ(p->region_for({k0, 4}), 0);

    uint8_t k5000[4];
    hfile::write_be32(k5000, 5000);
    EXPECT_EQ(p->region_for({k5000, 4}), 5);

    uint8_t kmax[4];
    hfile::write_be32(kmax, 99999);
