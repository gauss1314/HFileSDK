...
static KeyValue make_kv(const char* row, const char* family, const char* qualifier, int64_t ts, const char* value)
{
...
TEST(KVSerialize, TagsLengthAlwaysPresent)
{
    ...
    // Tags length at offset: 4+4+key_len+1 = 4+4+(2+1+1+1+8+1)+1=... easier
    // just check encoded_size encoded_size =
    // 4+4+key_len+value_len+2+tags_len+mvcc_len For empty tags: +2 bytes for
    // tags_len=0 Find the tags_len field: after value bytes
    uint32_t key_len = read_be32(buf.data());
    uint32_t val_len = read_be32(buf.data() + 4);
...
TEST(KVCompare, OrderingRules)
{
    ...
    a.qualifier = q;
    a.timestamp = 100;
    a.key_type = KeyType::Put;
    a.value = v;
    b.row = r2;
    b.family = fam;
    b.qualifier = q;
    b.timestamp = 100;
    b.key_type = KeyType::Put;
    b.value = v;
