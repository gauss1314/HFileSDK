...
// rowKeyRule:
// "STARTTIME,0,false,10#IMSI,1,true,15#MSISDN,2,false,11,RIGHT,#$RND$,
// 3,false,4"
...
{
    auto [b, s] = RowKeyBuilder::compile("STARTTIME,0,false,10#IMSI,1,true,15#MSISDN,2,"
                                         "false,11,RIGHT,#$RND$,3,false,4");
    EXPECT(s.ok());
...
// --- B-13 regression: duplicate col_index in rule → concat (dedup is in
// converter)
