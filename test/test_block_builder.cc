...
static KeyValue make_simple_kv(std::vector<uint8_t>& rk_buf,
                               ...
                               const char* val)
{
    rk_buf.assign(reinterpret_cast<const uint8_t*>(row), reinterpret_cast<const uint8_t*>(row) + strlen(row));
    fam_buf = {'c', 'f'};
    q_buf.assign(reinterpret_cast<const uint8_t*>(q), reinterpret_cast<const uint8_t*>(q) + strlen(q));
    v_buf.assign(reinterpret_cast<const uint8_t*>(val), reinterpret_cast<const uint8_t*>(val) + strlen(val));
