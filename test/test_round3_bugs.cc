...
void test_varint_roundtrip()
{
    ...
    {
        uint8_t buf[10];
        int n = encode_varint64(buf, v);
        EXPECT(n >= 1 && n <= 10);
        uint64_t out = 0;
        int r = decode_varint64(buf, out);
        EXPECT(r >= 1);
        EXPECT_EQ(out, v);
    }
}
...
{
    uint8_t buf[10];
    int n = encode_varint64(buf, UINT64_MAX);
    EXPECT_EQ(n, 10);
    uint64_t out = 0;
    int r = decode_varint64(buf, out);
    EXPECT_EQ(r, 10);
...
{
    // 11 bytes all with continuation bit set → loop must terminate at 10,
    // return -1
    uint8_t bad[11];
    std::memset(bad, 0x80, 11);
    uint64_t out = 0;
...
void test_region_for_correctness()
{
    ...
    auto k = [](char c)
    {
        return std::vector<uint8_t>{static_cast<uint8_t>(c)};
    };
...
void test_region_binary_keys()
{
    ...
    {
        uint8_t k[4];
        write_be32(k, static_cast<uint32_t>(i * 1000));
    ...
    uint8_t k5[4];
    write_be32(k5, 5000);
    EXPECT_EQ(p->region_for({k5, 4}), 5);
    uint8_t kx[4];
    write_be32(kx, 99999);
    EXPECT_EQ(p->region_for({kx, 4}), 9);
}
...
static KeyValue
make_kv(std::vector<uint8_t>& rk, std::vector<uint8_t>& fam, std::vector<uint8_t>& q, std::vector<uint8_t>& v)
{
    rk = {'r'};
    fam = {'c', 'f'};
    q = {'c'};
